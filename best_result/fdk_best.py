...
angle_scale = 2 * np.pi
detector_size = 0.5
detector_scale = 256 * 3
source_distance = 256 * 4
dector_distance = 256 * 4
...
plt.axis("off")
plt.savefig('best_proj.png')

# visualize detector data
det = np.transpose(projdata, (1, 0, 2))
det = det[np.arange(0, angle_num, int(angle_num / 100)), np.newaxis, :, :]
det_tensor = torch.from_numpy(det)
det_grid = np.transpose(vutils.make_grid(det_tensor, nrow=10, normalize=True), (1, 2, 0))
plt.imshow(det_grid)
plt.axis("off")
plt.savefig('best_det.png')
